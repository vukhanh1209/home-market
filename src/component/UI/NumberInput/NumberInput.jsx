...
const NumberInput = (props) => {
    ...
    return (
        <div className="flex h-fit items-center gap-x-1 text-sm font-semibold">
            ...
            <div style={{height: `${height}`, width: `${width}`, borderColor: `${border}`,borderRadius: `${borderRadius}` }} className={`flex items-center justify-center border border-transparent text-primary bg-white rounded-md `} value={`${value}`}>{value > 0 ? value : ''}</div>
            <button onClick={() => setValue(prev => prev + 1)} style={{backgroundColor: `${color}` , height:`${height}`, width:`${height}`, borderColor: `${border}`,borderRadius: `${borderRadius}`}} className="flex justify-center border border-transparent items-center bg-[#E6E5EA] text-[#919096] rounded-md">
                <RiAddFill />
            </button>
        </div>
